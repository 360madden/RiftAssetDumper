...
One file to consume them all: **`flythrough-index.json`** (118.5 KB)

| Artifact | Count | FT Phase |
|---|---|---|
| PNG textures | 12,954 | FT-1 |
| OBJ files | 350 (217 unique) | FT-2 |
| Per-OBJ metadata sidecars | 1 (emitter proven) | FT-3 |
| World transforms (world.json) | 217 (100% coverage) | FT-4 |
| Single-command pipeline | `bulk_export_for_flythrough.py` | FT-5 |
| Validation suite | ft6_validation.py (5 checks, 25 tests) | FT-6 |
| LOD manifest | 47 assets classified (7 families) | FT-7 |
...
| FT-6 | Validation suite | ✅ | 5 check categories, 0 errors |
| FT-7 | Zones + LOD | ✅ | LOD: 47 assets, 7 families; Zones: negative (render config) |
...
| `flythrough/lod-manifest.json` | LOD variant classification |
| `flythrough/scene-graph-manifest.json` | World.json index |
| `evidence/ft7.2/EVIDENCE.md` | LOD detection findings |
| `evidence/ft7.1/EVIDENCE.md` | Zone metadata (negative) |
