...
**Last Updated**: 2026-06 (Phase 4 ACTIVE — M4.1-M4.6 COMPLETE: descriptor-aware parser; M4.7 planning)

## Current Phase
**Phase 4: Descriptor-Aware Parser**
...
**M4.7** — Next descriptor-aware parser behavioral change (planning): add warning when byte-3 ≠ 0x00 in `AnalyzeNifDataStreamLayout`. Smallest possible behavioral change (~5 lines), uses best-proven Phase 2 evidence (byte-3=0x00 universal in 184/184 sampled blocks).
...
## Next Concrete Actions (Phase 4 M4.7)
1. Plan next descriptor-aware parser behavioral change
2. Implement with targeted tests
3. Build, test, format, code-review
4. Create Phase 4 progress handoff
...
- **Phase 4 (ACTIVE)**: M4.1 COMPLETE (byte-3 integrity), M4.2 COMPLETE (byte-0 fallback), M4.3 COMPLETE (JSON distribution), M4.4 COMPLETE (console summary), M4.5 COMPLETE (descriptor-role cross-check), M4.6 COMPLETE (stream-body probe visibility). 29/29 tests. M4.7 planning. — scope first parser behavioral change consuming descriptor fields. (`docs/handoffs/2026-06-m3.1-parser-descriptor-read-decision.md`). Formal per-template evaluation: 6 promotion gates still blocked; both flags false. Scope: descriptor read + classification only — no behavioral change. Decision: candidate-only. Live status refreshed. Test plan: 5 targeted xUnit cases. Next: implement the change in `Program.cs`.