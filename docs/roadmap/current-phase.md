...
**Last Updated**: 2026-06 (Phase 5 ACTIVE — M5.1 COMPLETE: descriptor on pairing records; M5.2 COMPLETE: confidence adjustment; M5.3 planning)
...
**M5.3** — Descriptor-based stream pre-filter (planning): add warning when byte-3 ≠ 0x00 in `AnalyzeNifDataStreamLayout`. Smallest possible behavioral change (~5 lines), uses best-proven Phase 2 evidence (byte-3=0x00 universal in 184/184 sampled blocks).
...
## Next Concrete Actions (Phase 5 M5.3)
1. Implement descriptor-based position stream pre-filter in decode-nif-geometry
