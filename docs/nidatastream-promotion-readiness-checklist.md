...
| Descriptor/sample comparison | One machine-readable report joins descriptor-helper readiness, sample corpus metadata, copied-sample byte-counter uniformity, candidate byte-order offsets, first descriptor-record byte distributions, record byte-0 index proof, helper argument-use proof, byte-role candidates, per-pattern matrix rows, copied-sample context correlation/review queue, and static-vs-stream semantic feasibility before any parser patch | Guarded/report-only by `nidatastream-descriptor-sample-compare`; summarized by `nidatastream-promotion-status` and `nidatastream-promotion-dashboard`; byte 0 is candidate-mapped as the static-table index, tracked helpers do not use bytes 1-2 for helper table lookup, byte 3 is a uniform-zero padding/reserved/sign-guard candidate, and descriptor records are grouped/ranked with copied-sample first pair record plus usage/access/type context, but variable bytes 1-2 remain unmapped for parser/export semantics, semantic mapping remains false, and promotion still blocks |
