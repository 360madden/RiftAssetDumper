...
| Descriptor/sample comparison | One machine-readable report joins descriptor-helper readiness, sample corpus metadata, copied-sample byte-counter uniformity, candidate byte-order offsets, and first descriptor-record byte distributions before any parser patch | Guarded/report-only by `nidatastream-descriptor-sample-compare`; summarized by `nidatastream-promotion-status` and `nidatastream-promotion-dashboard`; still blocks promotion |
