...
| Descriptor reference semantics | Ghidra reference kinds, memory-block metadata, and instruction text are classified before revising table stride/base assumptions or expanding sample scans | `nidatastream-descriptor-reference-classify`; schema `docs/schemas/ghidra-descriptor-reference-classification-v1.schema.json` | Report-only; current retained-project classifier found 20 DATA/address-like refs across 3 fields and 6 functions, enough to justify a candidate stride/base-model review but not parser/export promotion |
| Descriptor base/stride model review | Reference classification and table-sample status are joined before any new sample plan or parser proposal | `nidatastream-descriptor-base-model-review`; schema `docs/schemas/nidatastream-descriptor-base-model-review-v1.schema.json` | Report-only; candidate models can guide resampling but cannot promote parser/export behavior |
