...
| Descriptor/sample comparison | One machine-readable report joins descriptor-helper readiness, sample corpus metadata, copied-sample byte-counter uniformity, candidate byte-order offsets, first descriptor-record byte distributions, record byte-0 index proof, and static-vs-stream semantic feasibility before any parser patch | Guarded/report-only by `nidatastream-descriptor-sample-compare`; summarized by `nidatastream-promotion-status` and `nidatastream-promotion-dashboard`; byte 0 is candidate-mapped as the static-table index, but semantic mapping remains false and still blocks promotion |
