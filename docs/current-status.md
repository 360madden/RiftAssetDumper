...
| Ghidra / NiDataStream static proof | 🔬 first pass | Retained-project Ghidra surveys identify `NiDataStream::LoadBinary()` field/payload loading and a mesh semantic-adapter validator called from the DX9 material-binding path. This supports preserving stream element order and semantic grouping evidence; no parser behavior change is recommended yet. |
