...
> ⛒ **Contribution path:** Open a PR, wait for the **6 CI checks** (5 leaf jobs — `Docs Lint`, `.NET Build & Test`, `Python Lint & Test`, `Orphan Guard Regression`, `Pre-commit Checks` — plus the `Summary` aggregator) to go green, then `gh pr merge --squash --delete-branch`. Direct-push to `main` is a documented fallback.
...
Current stage terminology: the historical geometry/export pipeline is **Phase 49 complete** (350 OBJs, 0 unknowns). Active work is the **post-Stage-18 Ghidra/NiDataStream proof-guard lane** — all 7 promotion gates CLEARED, both flags true. Ghidra evidence stays candidate-only and does not drive parser/export behavior changes.
...
`scripts/ghidra/FunctionSiteSurvey.java` emits the current reusable function-site JSON shape; `docs/schemas/ghidra-function-site-survey-v1.schema.json` documents that generated report contract. Reusable target definitions live in `docs/ghidra-function-site-targets.json` and are schema-checked by `docs/schemas/ghidra-function-site-targets-v1.schema.json`; use `ghidra-function-site-target-guard` to fail closed on unsafe/duplicated report paths, `ghidra-function-site-survey --list-json` for an agent-readable target list, `ghidra-function-site-status --list-json` to inspect ignored report/summary presence, and `ghidra-function-site-survey --ghidra-target <key>` to print serialized rerun/summarize commands. `--list-json` mode still runs the generated-output guard but suppresses success banners so stdout is parseable JSON. Add `--ghidra-execute` only when intentionally running one target against the retained project. Use `ghidra-summarize` for reviewable Markdown summaries instead of committing raw `Exports/ghidra-reports/*.json`.
