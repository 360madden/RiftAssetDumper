...
**Completed lane — Navmesh Navigation** (`docs/roadmap/navmesh-navigation-roadmap.md`): **Phases 0-6 complete.** M6.1 records full/selected scope and SHA-256 input provenance, protects the canonical index, detects stale inputs, performs Draft-07 validation, and builds all 4 eligible cohorts. M6.2 produces a schema-validated 4-node/2-edge graph from actual navmesh-vertex proximity. M6.3 exposes weighted cross-zone A* through `navmesh_pathfind.py --cross-zone`, projects transitions onto both adjacent Detour meshes, and concatenates per-zone paths. M6.4 validates three reachable pairs below 1 ms and explicitly reports three disconnected nature-cohort pairs. CI runs the broad fresh-checkout-safe pytest suite; nine generated/live-local modules remain local validation only. NM-7 remains optional and safety-gated. Completion handoff: `docs/handoffs/2026-07-12-nm-6-phase6-completion.md`.
