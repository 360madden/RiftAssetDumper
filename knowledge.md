...
**Completed lane — Navmesh Navigation** (`docs/roadmap/navmesh-navigation-roadmap.md`): **COMPLETE.** Phases 0-6 delivered. M6.1 records full/selected scope and SHA-256 input provenance, protects the canonical index, detects stale inputs, performs Draft-07 validation, and builds all 4 eligible cohorts. M6.2 produces a schema-validated 4-node/2-edge graph from actual navmesh-vertex proximity. M6.3 exposes weighted cross-zone A* through `navmesh_pathfind.py --cross-zone`, projects transitions onto both adjacent Detour meshes, and concatenates per-zone paths. M6.4 validates three reachable pairs below 1 ms and explicitly reports three disconnected nature-cohort pairs. CI runs the broad fresh-checkout-safe pytest suite; nine generated/live-local modules remain local validation only. NM-7 skipped (movement injection contradicts read-only mandate — matches FT-8 precedent). Closure handoff: `docs/handoffs/2026-07-12-nm-6-phase6-completion.md`.
...
| `python scripts/synthesize_struct_layout.py [--validate]` | Synthesize `struct-layout-catalog.json` from ModRM + Phase 2 + Ghidra evidence |
...
- **Tests:** xUnit in `src/RiftAssetDumper.Tests/` (56 tests, all pass — includes 4 NifSceneGraph record smoke tests for FT-4.2)
...
| Phase | Topic | Status |
|-------|-------|:---:|
| NM-0 | Recast feasibility, geometry audit, walkability classification | ✅ DONE |
...
- **meshSize=297 TEXCOORD discovery** (2026-06-18): Residual stream at offset=24, labeled TEXCOORD by the string heuristic, proved to be `float32xvec3` position/normal/UV vertex data. **17 OBJs exported (55,805 vertices, 55,795 faces, 0 issues)** across 10 assets. Standouts: `0910220376b18d36` block #7 at 38,957v/38,955f (largest single-block export in project), and `9f32d26c425ed264` with all 8 NiMesh blocks decoded (see `docs/handoffs/2026-06-18-mesh297-discovery.md` for per-block breakdown). 374 mesh blocks in family.
- **meshSize=321 lighthouse discovery** (2026-06-18): Residual stream at offset=204 labeled POSITION (plausible=0.8947, 19 float3 vectors, 228 bytes) in `b89ced7d511388d2` (lighthouse model, 522 blocks, 27 NiMesh blocks). **10 OBJs exported (9,570 vertices, 9,550 faces, 0 issues)**. Largest blocks: #11=2,506v/2,504f, #41=2,539v/2,537f, #189=2,539v/2,537f, #425=1,776v/1,774f. OBJs at `Exports/discovery-plan/mesh321-probe/`.
...
- **Stage8 RiftFlythrough delivery (v0.2)** — `scripts/build_riftflythrough_delivery.py` filters 153 consumer-ready assets into `riftflythrough-delivery.json` (14,696 vertices, 23,634 faces, 404 linked textures, 19 mesh families) + markdown report. v0.2 (2026-06-17) drops absolute Windows paths from emitted JSON (privacy + browser-portable), resolves `linked_textures` basenames to `textures/converted/<file>.png` URLs (404/404 resolved, 100%), adds `_assert_no_absolute_paths()` hard guard. Copied to `C:\RIFT MODDING\RiftFlythrough\js\riftflythrough-delivery.json`. Tests: `tests/test_build_riftflythrough_delivery.py` (5 tests, all pass).
