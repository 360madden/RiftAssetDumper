...
with TemporaryDirectory() as temp_dir:
    ...
    check("second uint32", block["SecondUInt32"], 123)
    check("pair records offset", block["PairRecordsOffset"], 12)
    check("first pair record bytes", block["FirstPairRecordBytes"], "04 00 00 00 05 00 00 00")
    check("descriptor count offset", block["DescriptorCountOffset"], 20)
    check("descriptor records offset", block["DescriptorRecordsOffset"], 24)
    check("first descriptor record bytes", block["FirstDescriptorRecordBytes"], "aa 00 00 00")
    check("legacy offset", block["LegacyPayloadOffset"], 29)
    check("ghidra offset", block["PayloadPrefixBytes"], 28)
    check("payload end offset", block["PayloadEndOffset"], 36)
