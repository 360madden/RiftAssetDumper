...
MTL while preserving material assignments and copying referenced textures into
the combined package by default.
...
DEFAULT_TEXTURE_COPY_ROOT = DEFAULT_PACKAGE_ROOT / "textures"
...
def rewrite_mtl_texture_path(
    line: str,
    *,
    source_mtl: Path,
    output_mtl: Path,
    texture_copy_root: Path | None = None,
    copied_textures: dict[str, str] | None = None,
    ...
    if texture_copy_root is not None:
        copied_ref = copy_texture_for_package(
            texture_path,
            output_mtl=output_mtl,
            texture_copy_root=texture_copy_root,
            copied_textures=copied_textures if copied_textures is not None else {},
            missing_textures=missing_textures if missing_textures is not None else [],
        )
        if copied_ref is not None:
            return " ".join([*parts[:-1], copied_ref])
...
def copy_texture_for_package(
    texture_path: Path,
    *,
    output_mtl: Path,
    texture_copy_root: Path,
    copied_textures: dict[str, str],
    missing_textures: list[str],
) -> str | None:
    if not texture_path.exists():
        missing_textures.append(str(texture_path))
        return None

    dest = texture_copy_root / "converted" / texture_path.name
    dest_key = str(dest.resolve()).lower()
    source_key = str(texture_path.resolve())
    existing_source = copied_textures.get(dest_key)
    if existing_source is not None and existing_source != source_key:
        digest = sha1_file(texture_path)[:8]
        dest = texture_copy_root / "converted" / f"{texture_path.stem}_{digest}{texture_path.suffix}"
        dest_key = str(dest.resolve()).lower()

    if dest_key not in copied_textures:
        dest.parent.mkdir(parents=True, exist_ok=True)
        if dest.resolve() != texture_path.resolve():
            shutil.copy2(texture_path, dest)
        copied_textures[dest_key] = source_key

    return posixpath.relpath(_to_posix(dest.resolve()), _to_posix(output_mtl.parent.resolve()))
...
def collect_mtl_lines(
    source_mtls: list[Path],
    *,
    output_mtl: Path,
    texture_copy_root: Path | None = None,
...
def build_combined_obj_package(
    *,
    ...
    resolved_texture_copy_root = texture_copy_root or (mtl_out.parent / DEFAULT_TEXTURE_COPY_ROOT.name)
    effective_texture_copy_root = resolved_texture_copy_root if copy_textures else None
    mtl_lines, missing_mtls, copied_textures, missing_textures = collect_mtl_lines(
        [mtl for _, _, mtl in selected],
        output_mtl=mtl_out,
        texture_copy_root=effective_texture_copy_root,
    )
    obj_lines = [
    ...
    report = {
        ...
        "inputs": {
            ...
            "texture_copy_root": repo_relative_path(resolved_texture_copy_root, repo_root=repo_root)
            if copy_textures
            else None,
        },
        "outputs": {
            ...
            "textures": repo_relative_path(resolved_texture_copy_root, repo_root=repo_root) if copy_textures else None,
        },
        "summary": {
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- OBJ: `{outputs['obj']}`",
        f"- MTL: `{outputs['mtl']}`",
        *([f"- Textures: `{texture_output}`"] if texture_output else ["- Textures: not copied (`--no-copy-textures`)"]),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--texture-copy-root", type=Path)
    parser.add_argument(
        "--no-copy-textures",
        action="store_true",
        help="Leave MTL texture paths pointing at the shared flythrough texture tree.",
    )
    parser.add_argument(
        "--no-point-clouds",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_combined_obj_package(
        ...
        texture_copy_root=args.texture_copy_root,
    )
    summary = report["summary"]
    print(
        ...
        f"point_entries={summary['point_directive_entries']} textures={summary['copied_texture_files']} "
        f"verify={summary['verify_pass']} "
