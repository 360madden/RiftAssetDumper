        ...
        notes.append(
            {
                ...
                "obj_path": source_asset.get("obj_path") if isinstance(source_asset, dict) else None,
...
def build_texture_fallback_provenance_report(
    *,
    ...
    for entry in _fallback_rows(manifest):
            ...
            fallback_ref_reports.append(
                {
                    ...
                    "best_current_interpretation": (
                        "replacement texture comes from a flythrough-index asset with the same mesh signature"
                        if has_same_mesh_source
    ...
    return {
        ...
        "summary": {
            "fallback_rows": len(_fallback_rows(manifest)),
...
def _format_source_assets(notes: list[dict[str, Any]]) -> str:
    ...
    formatted = []
    for note in notes:
        suffix = "same mesh" if note.get("same_mesh_signature") else "different mesh"
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Fallback refs",
        "",
        "| Row | Target asset | Missing DDS | Replacement | Source asset evidence | Target mesh signature | Durable truth | Next action |",
        "|---:|---|---|---|---|---|---:|---|",
    ]
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            "- Same-mesh source assets make a fallback stronger for practical review/import usability, but they do not prove the missing DDS was recovered.",
