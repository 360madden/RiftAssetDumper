...
def _build_obj_header(total_assets: int) -> list[str]:
    """Build the header lines stamped at the top of the merged OBJ.

    The Producer line is sourced from `rift_workflow_utils.producer_version()`
    so the consumer (RiftFlythrough) can verify which RiftAssetDumper build
    produced the merged-OBJ artifact. Stamped at emission time, not at
    consumer-side load time, so the provenance is locked in once per
    regeneration.
    """
    return [
        "# World-placed merged OBJ — generated by build_world_placed_merge.py\n",
        f"# Producer: RiftAssetDumper {producer_version()}\n",
        f"# {total_assets} assets indexed\n",
        "#\n",
    ]
...
def main() -> None:
    ...
    lines_out: list[str] = _build_obj_header(total_assets)
