...
def test_merge_dedups_existing_and_appends_new(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(_merge_full_scan_links, "FLYTHROUGH_INDEX", tmp_path / "flythrough-index.json")
    ...
    assert all(json.loads(rec)["ModelIdPrefix"] != "xxx99xxx99xxx99x" for rec in lines), (
        "Out-of-subset entry leaked into baseline"
    )


def test_merge_tolerates_bom_in_scratch_and_baseline(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    scratch_file.write_bytes(
        ...
        + b"\xef\xbb\xbf"
        + b'{"ModelIdPrefix": "aaa11aaa11aaa11a", "Reference": "gamma.dds"}\n'
    )
...
def test_merge_dry_run_does_not_write(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_merge_logs_unparseable_lines_to_stderr(
    ...
    assert "skipping unparseable line" in captured.err, f"Expected JSONDecodeError stderr-log, got: {captured.err!r}"
...
def test_merge_emits_saturation_warning_on_zero_new_entries(
    ...
    monkeypatch.setattr(
        "sys.argv", ["_merge_full_scan_links", "--baseline", str(baseline), "--scratch", str(scratch_dir), "--dry-run"]
    )
    rc = _merge_full_scan_links.main()
    assert rc == 0
    captured = capsys.readouterr()
    assert "added 0 entries" in captured.err, f"Saturation regression: expected stderr warning, got: {captured.err!r}"


def test_merge_cleans_up_tmp_file_on_atomic_write(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
